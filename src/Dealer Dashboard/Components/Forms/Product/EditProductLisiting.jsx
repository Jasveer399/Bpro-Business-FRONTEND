...
import { addProductAsync, editProductAsync, fetchProductsAsync, setProduct } from "../../../../Redux/Features/productSlice";
...
function EditProductLisiting() {
    ...
    useEffect(() => {
        if (product) {
            ...
            setValue('date', product.expiryDate ? new Date(product.expiryDate).toISOString().split('T')[0] : '');
        ...
        try {
            ...
            imageContainers.forEach((container, index) => {
                if (container.file) {
                    ...
                    formDataToSend.append(`existingImages[${index}]`, container.existingImage);
                }
            });
            ...
            const response = await dispatch(editProductAsync({ productId: id, productData: formDataToSend }));
                        ...
                        <div className="w-[90%] flex gap-5">
                            ...
                            <TimeSelectorHour_Minutes
                                value={product?.expiryDate ?
                                    `${new Date(product.expiryDate).getHours().toString().padStart(2, '0')}:${new Date(product.expiryDate).getMinutes().toString().padStart(2, '0')}`
                                    : '00:00'
                                }
                        ...
                        <div className="w-[90%]">
                            ...
                            <div className="grid grid-cols-3 gap-5 w-full mt-5">
                                {PaymentOptions.map((option) => (
                                    ...
                                    >
                                        <Checkbox
                                            checked={paymentMethods.includes(option.value.toLowerCase()) ||
                                            ...
                                            onChange={(e) => {
                                                if (e.target.checked) {
                                                    ...
                                                    setPaymentMethods(prev =>
                                                        prev.filter(method =>
                                                            method !== option.value && method !== option.name
                                                        )
                                                    );
                                                }
                                            }}
                                    ...
                                    <div className="aspect-video rounded-xl shadow-[0_6px_15px_rgba(0,0,0,0.25)] hover:shadow-[0_8px_20px_rgba(0,0,0,0.25)] overflow-hidden">
                                        {container.file ? (
                                            ...
                                            <div className="relative h-full">
                                                ...
                                                <button
                                                    type="button"
                                                    onClick={() => removeImage(container.id, container.existingImage)}
                                                    className="absolute top-2 right-2 p-1 bg-red-500 rounded-full text-white opacity-0 group-hover:opacity-100 transition-opacity"
                                                >
                                                    <X size={16} />
                                                </button>
